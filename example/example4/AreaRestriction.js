 ...
 * 则地图浏览者只能在限定区域内浏览地图。
 * 基于Baidu Map API 1.2。
 * http://api.map.baidu.com/library/AreaRestriction/1.2/docs/help.html
 *
 * @author lihuiyin128@l63.com
 * @version 1.3
 */
...
(function() {
    ...
    AreaRestriction._mapMoveendEvent = function(e) {
        ...
        _st = setTimeout(function() {
            var point = e.target.gf;
            _array.push(center);
            if (_array.length >= _num) {
                _array.shift();
                if (_array[_num - 2].lng == _array[_num - 3].lng && _array[_num - 2].lat == _array[_num - 3].lat) {
                    clearTimeout(_st);
                    return;
                }
            }
