...
(function() {
        ...
        var startAction = function(e) {
            ...
            if (points.length == 1) {
                if (me._drawingType == BMAP_DRAWING_POLYLINE) {
                    ...
                    newOverlay = new BMap.Polygon(drawPoint, me.polygonOptions);
                }
                map.addOverlay(newOverlay);
                map.addOverlay(overlay);
            } else {
                overlay.setPath(drawPoint);
                newOverlay.setPath(drawPoint);
            }
            ...
            if (!isBinded) {
                ...
                //计算面积
                newOverlay.setPath(points);
                newOverlay.hide();
                var calculate = me._calculate(newOverlay, points[points.length - 1]);
            }
     ...
     * @return {[type]}     [number]
     */
    DrawingManager.prototype._milliFormat = function(num) {
        return num && num.toString().replace(/(^|\s)\d+/g, function(m) { return m.replace(/(?=(?!\b)(\d{3})+$)/g, ',') });
    }
    /**
     * 添加显示所绘制图形的面积或者长度
     * @param {overlay} 覆盖物
    ...
    DrawingManager.prototype._calculate = function(overlay, point) {
        ...
        if (this._enableCalculate && BMapLib.GeoUtils) {
            ...
            if (result.data != 0) {
                result.label = this._addLabel(point, me._milliFormat((result.data / 1000000).toFixed(2)) + 'km²');
            }
        }
        return result;
    }
    ...
    DrawingManager.prototype._addLabel = function(point, content) {
        ...
        //删除之前的Label
        var over = this._map.getOverlays();
        for (var i = 0; i < over.length; i++) {
            if (over[i] instanceof(BMap.Label)) {
                if (over[i].point != null) {
                    this._map.removeOverlay(over[i]);
                }
            }
        }
